...
const Archive = () => {
  ...
  const { data: quote } = useQuery({
    queryKey: ["archive-quote", dayIndex, theme],
    queryFn: async () => {
  ...
  const { data: funFact } = useQuery({
    queryKey: ["archive-fact", dayIndex, theme],
    queryFn: async () => {
  ...
  const { data: word } = useQuery({
    queryKey: ["archive-word", dayIndex, theme],
    queryFn: async () => {
    ...
    <div className="min-h-screen bg-background">
      <nav className="sticky top-0 z-30 px-4 sm:px-12 bg-background/80 backdrop-blur-sm border-b border-border/50">
        <div className="max-w-3xl mx-auto flex items-center justify-between h-16">
          ...
          <p className="text-xs tracking-[0.3em] uppercase text-muted-foreground font-sans">Archive</p>
          <div className="w-16" />
        </div>
      </nav>

      <div className="max-w-3xl mx-auto px-4 sm:px-12 pt-8">
        <ThemeSelector />
      </div>

      <div className="flex items-center justify-center gap-6 py-10 sm:py-16">
        <button onClick={() => setOffset((o) => Math.min(maxOffset, o + 1))} disabled={offset >= maxOffset} className={`p-2.5 border border-border rounded-full transition-all ${offset >= maxOffset ? "opacity-20 cursor-not-allowed" : "text-muted-foreground hover:text-foreground active:scale-90"}`} aria-label="Older">
          <ChevronLeft className="w-5 h-5" />
        </button>
        <div className="text-center min-w-[200px]">
          <p className="font-display text-xl sm:text-2xl text-foreground font-light">{formatDate(targetDate)}</p>
          <p className="text-xs text-muted-foreground mt-1 font-sans">{offset === 1 ? "Yesterday" : `${offset} days ago`}</p>
        </div>
        <button onClick={() => setOffset((o) => Math.max(1, o - 1))} disabled={offset <= 1} className={`p-2.5 border border-border rounded-full transition-all ${offset <= 1 ? "opacity-20 cursor-not-allowed" : "text-muted-foreground hover:text-foreground active:scale-90"}`} aria-label="Newer">
      ...
      <main className="max-w-3xl mx-auto px-4 sm:px-12 pb-24 space-y-20">
        {quote && (
          <section>
            ...
            <blockquote className="font-display text-2xl sm:text-3xl md:text-4xl text-foreground leading-[1.3] font-light italic">&ldquo;{quote.text}&rdquo;</blockquote>
            <p className="mt-6 text-sm tracking-[0.2em] uppercase text-muted-foreground font-sans font-light">— {quote.author}</p>
          </section>
        )}
        {funFact && (
          <section>
            ...
            <p className="font-display text-xl sm:text-2xl md:text-3xl text-foreground leading-relaxed font-light">{funFact.fact}</p>
        ...
        {word && (
          <section>
            ...
            <h3 className="font-display text-3xl sm:text-4xl md:text-5xl text-foreground mb-2 font-light italic">{word.word}</h3>
